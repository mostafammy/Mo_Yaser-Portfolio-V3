 ...
 * Keyboard
 *
 * A fully interactive, realistic on-screen keyboard with mechanical key-switch
 * sound effects. Each key press produces a unique sampled sound (down + up)
 * and visual feedback (shadow depth change + scale).
 *
 * When to use:
 * - Portfolio "about" / "toolbox" sections to showcase your dev setup or
 *   keyboard-centric aesthetic (mechanical keyboard enthusiasts).
 * - Interactive hero sections where you want a tactile, typewriter-like
 *   element that responds to both mouse clicks and physical keyboard input.
 * - Case studies or coding demos where you want a live keystroke preview
 *   (set `showPreview`) to display the last typed character.
 *
 * Features:
 * - Full 14-row Apple Magic Keyboard layout with accurate key sizing.
 * - **Sound engine**: loads a single OGG sprite (`/sounds/sound.ogg`) with
 *   per-key start/end offsets for authentic mechanical switch sounds.
 *   Enable via `enableSound` prop.
 * - Physical keyboard passthrough: when the component is visible in the
 *   viewport, physical keydown/keyup events are captured and mapped to the
 *   visual keys + sounds.
 * - Context API (`KeyboardProvider`) shares pressed-key state across the
 *   entire keyboard tree.
 * - Built with `motion/react` for spring-based animations.
 *
 * Props:
 *   className?: string       — wrapper classes (use for margin/positioning).
 *   enableSound?: boolean    — load and play mechanical key sounds.
 *   showPreview?: boolean    — show a floating keystroke preview above the keys.
 *
 * ⚠️ Sound file required at: `public/sounds/sound.ogg`
 *    The component degrades gracefully if the file is missing.
 *
 * Layout & scaling:
 *   The keyboard uses `zoom` for responsive sizing (0.8x → 2x across
 *   breakpoints). Wrap in a container with your desired max-width.
 */
...
const KeystrokePreview = () => {
  ...
  const displayKey = lastPressedKey && !["Space", "ShiftLeft", "ShiftRight"].includes(lastPressedKey)

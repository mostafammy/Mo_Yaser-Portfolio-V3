 ...
 *
 * A canvas-based animated background that renders a grid of dots
 * with organic shimmering glow effects. Each dot oscillates
 * independently (random phase + speed) between dim and bright,
 * creating a subtle "breathing" starfield or particle effect.
 *
 * When to use:
 * - Hero sections, cards, or panels that need a subtle animated backdrop
 * - Dark mode-friendly backgrounds with per-dot glow transitions
 * - Any container where you want a "techy" or "space-like" dotted texture
 *   that responds to the current color scheme via CSS variables
 *
 * Props:
 * - gap: distance between dot centers (default 12)
 * - radius: dot radius in CSS px (default 2)
 * - color / darkColor: dot fill color for light/dark mode
 * - glowColor / darkGlowColor: glow/shadow color when a dot is bright
 * - colorLightVar / colorDarkVar: optional CSS variable to resolve dot color
 * - glowColorLightVar / glowColorDarkVar: optional CSS variable for glow color
 * - opacity: global alpha for the whole layer (default 0.6)
 * - backgroundOpacity: radial fade from center (0 = transparent)
 * - speedMin / speedMax / speedScale: control oscillation speed range
 */

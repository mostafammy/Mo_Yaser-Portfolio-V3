/**
 * PillBase — 3D Adaptive Navigation Pill
 *
 * A premium, morphing navigation pill that collapses to show only the active
 * section label and expands on hover to reveal all nav items. Designed with
 * layered inset shadows, gloss reflections, and spring physics for a
 * first‑party Apple‑calibre UI feel.
 *
 * When to use:
 * - Fixed or sticky site‑wide navigation for portfolio case studies.
 * - Hero‑adjacent nav where minimal chrome is desired (collapsed = clean).
 * - Any section where you want a tactile, "physical" pill that responds
 *   to hover and selection with smooth spring motion.
 *
 * Props: none (state is self‑contained). Items are hardcoded; fork the
 *   `navItems` array to customise labels/ids.
 *
 * Behaviour:
 *   - Collapsed: shows current section label with entrance animation.
 *   - Hover: pill expands horizontally (140px → 580px) with staggered
 *     item reveal. A 600ms delay prevents accidental collapse.
 *   - Click: updates active section, collapses pill, transitions shadow.
 *   - Spring: `pillWidth` and `pillShift` use framer-motion springs for
 *     natural‑feeling width/position changes.
 *
 * Styling:
 *   - Multi‑layer glassmorphism: multiple `linear-gradient` and
 *     `radial-gradient` pseudo‑layers for real‑time specular highlights.
 *   - Inset shadow stack creates depth (edge ridge, contact shadow,
 *     inner diffuse glow, micro edge definition).
 *   - Text uses `text-shadow` stacking for a debossed/embossed effect.
 */
